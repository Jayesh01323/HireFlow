"""
Unstop Job Connector

Implementation for Unstop job scraping using web scraping.
"""
...
class UnstopConnector(BaseJobConnector):
    ...
    def search_jobs(
        ...
        logger.info(f"Searching Unstop for: {query}, location: {location}, remote: {remote}")
        ...
        try:
            ...
            logger.info(f"Found {len(jobs)} jobs on Unstop")
            return jobs[:limit]
            
        except Exception as e:
            logger.error(f"Error searching Unstop jobs: {type(e).__name__}: {e}")
            import traceback
    ...
    def _build_search_params(
        ...
        """Build search parameters for Unstop."""
        params = {
            "q": query,
        }
    ...
    def _parse_job_cards(self, html: str) -> List[JobPosting]:
        """Parse job cards from Unstop response HTML."""
        jobs: List[JobPosting] = []
    ...
    def _parse_single_card(self, card: Any) -> Optional[JobPosting]:
        """Parse a single Unstop job card."""
        try:
            # Extract job title
