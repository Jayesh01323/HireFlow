"""
Cutshort Job Connector

Implementation for Cutshort job scraping using web scraping.
"""
...
class CutshortConnector(BaseJobConnector):
    """Connector for Cutshort job postings."""
    
    def __init__(self) -> None:
        """Initialize Cutshort connector."""
        super().__init__("cutshort")
        self.base_url = "https://cutshort.io"
        self.search_url = "https://cutshort.io/jobs"
    ...
    def search_jobs(
        ...
        logger.info(f"Searching Cutshort for: {query}, location: {location}, remote: {remote}")
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"Cutshort returned status {response.status_code}")
                return jobs
            
            jobs = self._parse_job_cards(response.text)
            
            logger.info(f"Found {len(jobs)} jobs on Cutshort")
            return jobs[:limit]
            
        except Exception as e:
            logger.error(f"Error searching Cutshort jobs: {type(e).__name__}: {e}")
            import traceback
    ...
    def _build_search_params(
        ...
        """Build search parameters for Cutshort."""
        params = {
            "q": query,
        }
    ...
    def _parse_job_cards(self, html: str) -> List[JobPosting]:
        """Parse job cards from Cutshort response HTML."""
        jobs: List[JobPosting] = []
