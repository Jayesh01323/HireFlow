"""
Foundit Job Connector

Implementation for Foundit job scraping using web scraping.
"""
...
class FounditConnector(BaseJobConnector):
    """Connector for Foundit job postings."""
    
    def __init__(self) -> None:
        """Initialize Foundit connector."""
        super().__init__("foundit")
        self.base_url = "https://www.foundit.in"
        self.search_url = "https://www.foundit.in/jobs"
    ...
    def search_jobs(
        ...
        logger.info(f"Searching Foundit for: {query}, location: {location}, remote: {remote}")
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"Foundit returned status {response.status_code}")
                return jobs
            ...
            logger.error(f"Error searching Foundit jobs: {type(e).__name__}: {e}")
            import traceback
    ...
    def _build_search_params(
        ...
        """Build search parameters for Foundit."""
        params = {
            "q": query,
        }
    ...
    def _parse_job_cards(self, html: str) -> List[JobPosting]:
        """Parse job cards from Foundit response HTML."""
        jobs: List[JobPosting] = []
    ...
    def _parse_single_card(self, card: Any) -> Optional[JobPosting]:
        ...
        try:
            ...
            # Extract URL
            link_elem = card.find('a', class_='jobLink')
    ...
    def get_job_details(self, job_url: str) -> Optional[JobPosting]:
        ...
        try:
            ...
            # Extract description
            desc_elem = soup.find('div', class_='jobDescription')
