...
Uses Apify actor for reliable LinkedIn job scraping.
"""
...
class LinkedInConnector(BaseJobConnector):
    """Connector for LinkedIn job postings via Apify."""
    
    def __init__(self) -> None:
        """Initialize LinkedIn connector."""
        super().__init__("linkedin")
        self.api_key = get_env("APIFY_API_KEY", "")
        self.actor_id = "automation-lab/linkedin-jobs-scraper"
    ...
    def search_jobs(
        ...
        """Search for jobs on LinkedIn via Apify."""
        logger.info(f"Searching LinkedIn via Apify for: {query}, location: {location}, remote: {remote}")
        ...
        try:
            ...
            logger.info(f"Found {len(jobs)} jobs via LinkedIn Apify connector")
            return jobs[:limit]
            ...
            logger.error(f"Error searching LinkedIn jobs via Apify: {type(e).__name__}: {e}")
            import traceback
    ...
    def get_job_details(self, job_url: str) -> Optional[JobPosting]:
        ...
        logger.info(f"LinkedIn get_job_details not implemented for: {job_url}")
        return None
